...
class AlertManager(
     ...
     */
    private fun handleAlertTriggered(rule: AlertRule, currentValue: Double, existingAlert: AlertInstance?) {
        val alertKey = "${rule.name}-${rule.severity}"
        ...
        logger.warn("Alert triggered: ${rule.name} (${rule.severity}) - Current: $currentValue, Threshold: ${rule.threshold}")
     ...
     */
    private fun handleAlertResolved(rule: AlertRule, existingAlert: AlertInstance?) {
        if (existingAlert != null) {
            val alertKey = "${rule.name}-${rule.severity}"
            ...
            if (rule.notifyOnResolve) {
                val notification = NotificationTask(
                    alert = existingAlert,
    ...
    private fun scheduleNotifications(alert: AlertInstance) {
        ...
        val immediateNotification = NotificationTask(
            alert = alert,
            channels = rule.notificationChannels,
        ...
        rule.escalationLevels.forEach { escalation ->
            executorService.schedule({
                if (activeAlerts.containsKey("${rule.name}-${rule.severity}")) {
                    val escalationNotification = NotificationTask(
                        alert = alert,
     ...
     */
    private fun sendEmailNotification(notification: NotificationTask, channel: NotificationChannel) {
     ...
     */
    private fun sendSlackNotification(notification: NotificationTask, channel: NotificationChannel) {
     ...
     */
    private fun sendWebhookNotification(notification: NotificationTask, channel: NotificationChannel) {
     ...
     */
    private fun sendSmsNotification(notification: NotificationTask, channel: NotificationChannel) {
     ...
     */
    private fun sendPagerDutyNotification(notification: NotificationTask, channel: NotificationChannel) {
     ...
     */
    fun suppressAlert(ruleName: String, severity: AlertSeverity, durationMinutes: Int) {
