...
class BackpressureManager(
    ...
    private fun <T> bufferEvent(
        ...
        val bufferedEvent = BufferedEvent(
            event = event,
            priority = priority,
    ...
    private fun shouldApplyBackpressure(): Boolean {
        ...
        val permitUtilization = ((config.maxConcurrentOperations - operationSemaphore.availablePermits()).toDouble() / 
                               config.maxConcurrentOperations.toDouble()) * 100
    ...
    fun getStatistics(): BackpressureStatistics {
        ...
        val rejectionRate = if (totalRequests.get() > 0) {
            (rejectedRequests.get().toDouble() / totalRequests.get().toDouble()) * 100
        } else 0.0
        
        val dropRate = if (totalRequests.get() > 0) {
