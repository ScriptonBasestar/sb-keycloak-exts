...
class RedisEventListenerProvider(
    ...
    private val batchProcessor: BatchProcessor<RedisEventMessage>,
) : EventListenerProvider {
    private val logger = Logger.getLogger(RedisEventListenerProvider::class.java)
    ...
    override fun onEvent(event: Event) {
        ...
        try {
            ...
            val fields: Map<String, String> =
                mapOf(
                    "id" to keycloakEvent.id,
                    "type" to keycloakEvent.type,
                    "realmId" to (keycloakEvent.realmId ?: ""),
                    "userId" to (keycloakEvent.userId ?: ""),
                    "data" to json,
                )

            // Send event with resilience patterns
            sendEventWithResilience(
                streamKey = config.userEventsStream,
                fields = fields,
            ...
            metrics.recordEventSent(
                eventType = event.type.name,
                realm = event.realmId ?: "unknown",
                destination = config.userEventsStream,
                sizeBytes = json.toByteArray().size,
            )
            metrics.stopTimer(timerSample, event.type.name)

            logger.debug("User event sent to Redis: type=${event.type}, userId=${event.userId}")
    ...
    override fun onEvent(
        ...
        try {
            ...
            val fields: Map<String, String> =
                mapOf(
                    "id" to keycloakAdminEvent.id,
                    "operationType" to keycloakAdminEvent.operationType,
                    "realmId" to (keycloakAdminEvent.realmId ?: ""),
                    "userId" to (keycloakAdminEvent.authDetails.userId ?: ""),
                    "resourcePath" to (keycloakAdminEvent.resourcePath ?: ""),
                    "data" to json,
                )

            // Send event with resilience patterns
            sendEventWithResilience(
                streamKey = config.adminEventsStream,
                fields = fields,
            ...
            metrics.recordEventSent(
                eventType = "ADMIN_${event.operationType.name}",
                realm = event.realmId ?: "unknown",
                destination = config.adminEventsStream,
                sizeBytes = json.toByteArray().size,
            )
            metrics.stopTimer(timerSample, "ADMIN_${event.operationType.name}")

            logger.debug("Admin event sent to Redis: type=${event.operationType}, userId=${event.authDetails.userId}")
        } catch (e: CircuitBreakerOpenException) {
            metrics.recordEventFailed(
                eventType = "ADMIN_${event.operationType.name}",
                realm = event.realmId ?: "unknown",
    ...
    override fun close() {
        ...
        logger.debug("RedisEventListenerProvider closed")
    }

    /**
     * Send event to Redis Stream with circuit breaker, retry policy, dead letter queue, and batch processing
     */
    ...
    private fun addToDeadLetterQueue(
        ...
        deadLetterQueue.add(
            ...
            metadata =
                mapOf(
                    "streamKey" to message.streamKey,
