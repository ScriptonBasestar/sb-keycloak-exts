...
class GitHubIdentityProvider(
    ...
    private fun getUserProfile(accessToken: String): JsonNode {
        ...
        val response = SimpleHttp.doGet(url, session)
    ...
    private fun verifyOrganizationMembership(accessToken: String, context: BrokeredIdentityContext) {
        if (config.requiredOrganization.isBlank()) return
        
        val orgsUrl = config.getApiUrl("/user/orgs")
        val organizations = SimpleHttp.doGet(orgsUrl, session)
            .header("Authorization", "Bearer $accessToken")
            .header("Accept", "application/vnd.github.v3+json")
            .header("User-Agent", "Keycloak GitHub Identity Provider")
            .asJson()
        ...
        if (!orgNames.contains(config.requiredOrganization)) {
            logger.warnf("User %s is not a member of required organization: %s", 
                context.username, config.requiredOrganization)
    ...
    private fun verifyTeamMembership(accessToken: String, context: BrokeredIdentityContext) {
        val teamsUrl = config.getApiUrl("/user/teams")
        val teams = SimpleHttp.doGet(teamsUrl, session)
            .header("Authorization", "Bearer $accessToken")
            .header("Accept", "application/vnd.github.v3+json")
            .header("User-Agent", "Keycloak GitHub Identity Provider")
            .asJson()
        ...
        if (!teamNames.contains(config.requiredTeam)) {
            logger.warnf("User %s is not a member of required team: %s", 
                context.username, config.requiredTeam)
    ...
    private fun verifySSHKeys(accessToken: String, context: BrokeredIdentityContext) {
        val sshKeysUrl = config.getApiUrl("/user/keys")
        val sshKeys = SimpleHttp.doGet(sshKeysUrl, session)
            .header("Authorization", "Bearer $accessToken")
            .header("Accept", "application/vnd.github.v3+json")
            .header("User-Agent", "Keycloak GitHub Identity Provider")
            .asJson()
        ...
        logger.debugf("SSH keys verification passed for user: %s (%d keys)", 
            context.username, sshKeys.size())
    }

    private fun verify2FAStatus(accessToken: String, context: BrokeredIdentityContext) {
        val userUrl = config.getApiUrl("/user")
        val userInfo = SimpleHttp.doGet(userUrl, session)
            .header("Authorization", "Bearer $accessToken")
            .header("Accept", "application/vnd.github.v3+json")
            .header("User-Agent", "Keycloak GitHub Identity Provider")
            .asJson()
    ...
    private fun syncTeamMemberships(accessToken: String, context: BrokeredIdentityContext) {
        try {
            val teamsUrl = config.getApiUrl("/user/teams")
            val teams = SimpleHttp.doGet(teamsUrl, session)
                .header("Authorization", "Bearer $accessToken")
            ...
            context.setUserAttribute("github_team_memberships", 
                teamMemberships.joinToString(",") { "${it["organization"]}/${it["team"]}:${it["permission"]}" })
            
            logger.debugf("Team memberships synced for user: %s (%d teams)", 
                context.username, teamMemberships.size)
        } catch (e: Exception) {
            logger.warn("Failed to sync team memberships", e)
        }
    }

    private fun syncRepositoryAccess(accessToken: String, context: BrokeredIdentityContext) {
        try {
            val reposUrl = config.getApiUrl("/user/repos?type=all&sort=updated&per_page=100")
            val repositories = SimpleHttp.doGet(reposUrl, session)
                .header("Authorization", "Bearer $accessToken")
                .header("Accept", "application/vnd.github.v3+json")
                .header("User-Agent", "Keycloak GitHub Identity Provider")
                .asJson()
            
            val repoAccess = repositories.map {
                mapOf(
            ...
            context.setUserAttribute("github_repository_access", 
                repoAccess.take(50).joinToString(",") { "${it["name"]}:${it["permission"]}" })
            
            logger.debugf("Repository access synced for user: %s (%d repos)", 
                context.username, repositories.size())
        } catch (e: Exception) {
            logger.warn("Failed to sync repository access", e)
        }
    }

    private fun enhanceDeveloperProfile(accessToken: String, context: BrokeredIdentityContext) {
        try {
            // Get user profile with additional stats
            val userUrl = config.getApiUrl("/user")
            val userProfile = SimpleHttp.doGet(userUrl, session)
                .header("Authorization", "Bearer $accessToken")
                .header("Accept", "application/vnd.github.v3+json")
                .header("User-Agent", "Keycloak GitHub Identity Provider")
                .asJson()
    ...
    private fun extractProgrammingLanguages(accessToken: String, context: BrokeredIdentityContext) {
        try {
            ...
            val languages = repositories.mapNotNull { repo ->
            ...
            logger.debugf("Programming languages extracted for user: %s (%s)", 
                context.username, languages.joinToString(", "))
        } catch (e: Exception) {
    ...
    private fun performEnterpriseIntegration(accessToken: String, context: BrokeredIdentityContext) {
        try {
    ...
    private fun syncEnterpriseOrganizationStructure(accessToken: String, context: BrokeredIdentityContext) {
        try {
            val orgsUrl = config.getApiUrl("/user/orgs")
            val organizations = SimpleHttp.doGet(orgsUrl, session)
                .header("Authorization", "Bearer $accessToken")
            ...
            val orgStructure = organizations.map { org ->
                val orgLogin = org.get("login").asText()
                
                // Get organization details
                val orgDetailsUrl = config.getApiUrl("/orgs/$orgLogin")
                val orgDetails = SimpleHttp.doGet(orgDetailsUrl, session)
                    .header("Authorization", "Bearer $accessToken")
                    .header("Accept", "application/vnd.github.v3+json")
                    .header("User-Agent", "Keycloak GitHub Identity Provider")
                    .asJson()
            ...
            context.setUserAttribute("github_enterprise_organizations", 
                orgStructure.joinToString("|") { "${it["login"]}:${it["name"]}" })
            
            logger.debugf("Enterprise organization structure synced for user: %s (%d orgs)", 
                context.username, orgStructure.size)
        } catch (e: Exception) {
            logger.warn("Failed to sync enterprise organization structure", e)
        }
    }

    private fun mapTeamsToKeycloakGroups(accessToken: String, context: BrokeredIdentityContext) {
        try {
            val teamsUrl = config.getApiUrl("/user/teams")
            val teams = SimpleHttp.doGet(teamsUrl, session)
                .header("Authorization", "Bearer $accessToken")
            ...
            val keycloakGroups = teams.map { team ->
            ...
            context.setUserAttribute("keycloak_group_mappings", 
                keycloakGroups.joinToString("|") { it["path"] as String })
            
            context.setUserAttribute("github_team_group_mappings",
                keycloakGroups.joinToString("|") { "${it["organization"]}/${it["name"]}:${it["path"]}" })
            
            logger.debugf("Teams mapped to Keycloak groups for user: %s (%d teams)", 
                context.username, keycloakGroups.size)
        } catch (e: Exception) {
            logger.warn("Failed to map teams to Keycloak groups", e)
        }
    }

    private fun configureRepositoryBasedAuthorization(accessToken: String, context: BrokeredIdentityContext) {
        try {
            val reposUrl = config.getApiUrl("/user/repos?type=all&sort=updated&per_page=100")
            val repositories = SimpleHttp.doGet(reposUrl, session)
                .header("Authorization", "Bearer $accessToken")
            ...
            val repoAuthorizations = repositories.map { repo ->
            ...
            context.setUserAttribute("github_admin_repositories", 
                adminRepos.joinToString(",") { it["repository"] as String })
            context.setUserAttribute("github_write_repositories", 
                writeRepos.joinToString(",") { it["repository"] as String })
            context.setUserAttribute("github_read_repositories", 
            ...
            logger.debugf("Repository-based authorization configured for user: %s (admin: %d, write: %d, read: %d)",
                context.username, adminRepos.size, writeRepos.size, readRepos.size)
        } catch (e: Exception) {
            logger.warn("Failed to configure repository-based authorization", e)
        }
    }

    private fun logUserAuthentication(context: BrokeredIdentityContext, accessToken: String) {
        try {
            val auditData = mapOf(
            ...
            logger.infof("GitHub authentication successful: user=%s, ip=%s, enterprise=%s", 
                context.username, 
                session.getContext().connection.remoteAddr,
                config.isGitHubEnterprise)
            
            // Store audit data for external systems
            context.setUserAttribute("github_audit_log", 
